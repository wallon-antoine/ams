...
class Ldap {
	...
	public function __construct(){
        ...
        private function connect() {
            ...
            $connect = ldap_connect($this->_CI->config->item('ldapuri'), $this->_CI->config->item('ldapport'));
            
            ldap_set_option($connect, LDAP_OPT_PROTOCOL_VERSION, 3);
            ldap_set_option($connect, LDAP_OPT_REFERRALS, 0);;
            ldap_bind($connect, $this->_CI->config->item('binddn'), $this->_CI->config->item('bindpw'));
        ...
        public function search_entry($filter,$type) {
            ...
            $read = ldap_search($this->connect(), "ou=People,".$this->_CI->config->item('basedn'), $type."=".$filter,$justthese);
        ...
        public function search($filter,$type) {
            ...
            foreach ($separateur AS $null => $person) {
                ...
                $filter="(|(uid=*$person*))";
                $justthese = array("sn", "givenname", "uid");
                $sr        = ldap_search($this->connect(),"ou=People,".$this->_CI->config->item('basedn'), $filter,$justthese);
